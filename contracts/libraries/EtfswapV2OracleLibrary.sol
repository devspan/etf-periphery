...
library EtfswapV2OracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = IEtfswapV2Pair(pair).price0CumulativeLast();
        price1Cumulative = IEtfswapV2Pair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IEtfswapV2Pair(pair).getReserves();
