...
library EtfswapV2LiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = EtfswapV2Library.getReserves(factory, tokenA, tokenB);

        require(reserveA > 0 && reserveB > 0, 'EtfswapV2ArbitrageLibrary: ZERO_PAIR_RESERVES');
        ...
        if (aToB) {
            uint amountOut = EtfswapV2Library.getAmountOut(amountIn, reserveA, reserveB);
            reserveA += amountIn;
            reserveB -= amountOut;
        } else {
            uint amountOut = EtfswapV2Library.getAmountOut(amountIn, reserveB, reserveA);
            reserveB += amountIn;
            reserveA -= amountOut;
        }
    }
    ...
    function getLiquidityValue(
        ...
        (uint256 reservesA, uint256 reservesB) = EtfswapV2Library.getReserves(factory, tokenA, tokenB);
        IEtfswapV2Pair pair = IEtfswapV2Pair(EtfswapV2Library.pairFor(factory, tokenA, tokenB));
        bool feeOn = IEtfswapV2Factory(factory).feeTo() != address(0);
